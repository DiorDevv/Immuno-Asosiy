...
class TranslateApiView(APIView):
    ...
    def post(request, lang):
        language = Languege.objects.get(name=lang) if Languege.objects.filter(name=lang).exists() else None
        if language is None:
            return Response({
...
class LanguageApiView(APIView):
    ...
    def post(request):
        ...
        if Languege.objects.filter(name=name).exists():
            return Response({
                'error': 'Language already exists'
            }, status=400)
        else:
            language = Languege.objects.create(name=name)
            return Response({
                'name': language.name
            })
    ...
    def delete(request):
        name = request.data.get('name')
        if Languege.objects.filter(name=name).exists():
            language = Languege.objects.get(name=name)
            language.delete()
            return Response({
                'success': True
            })
...
class TranslateAdminApiView(APIView):
    ...
    def get(request):
        ...
        all_languages = Languege.objects.all()
        data = []
        for key in all_keys:
            translate = {
                'key': key['key'],
